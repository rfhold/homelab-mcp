...
The repository implements hosted OAuth with Authentik browser authentication, stateless MCP, and the progressive read-only `grafana_query` tool with LogQL, PromQL, TraceQL, and Profiles actions. The application uses a concrete services composition root and integration-owned clients, actions, errors, and telemetry so additional integrations can be added without coupling MCP dispatch to their SDKs.
