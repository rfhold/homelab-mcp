...
The repository implements hosted OAuth with Authentik browser authentication, stateless MCP, the read-only `grafana_query` tool, and the operationally consequential `grafana_exec` tool. Grafana actions cover datasource queries, alert-rule and alert-instance reads, and bounded silence creation. The application uses a concrete services composition root and integration-owned clients, actions, errors, and telemetry so additional integrations can be added without coupling MCP dispatch to their SDKs.
