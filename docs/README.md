...
The repository implements hosted OAuth, authenticated `/mcp`, six read-only `grafana_query` actions, and `grafana_exec` action `silence.create`. The existing `mcp:use` scope authorizes every action on both tools.
