...
Sudo authentication is separate and lazy. After agent success, the sudo password is requested only when the first privileged operation consumes it. In the default mode, an SSH fallback password is cached in redacted memory and reused for sudo; otherwise sudo receives its own lazy prompt. `--separate-sudo-password` always uses an independent lazy sudo prompt. The deploy does not configure pyinfra's native sudo password, askpass, argument, or environment paths. The fixed sudoers installation uses an explicit `sudo -S -k -p ''` wrapper, validates the policy, and closes privileged-child stdin before installation work. Each subsequent native desired-state operation and the SSH trust transaction receives a fresh reusable password stdin and uses static pyinfra shell executable `sudo -S -k -p '' -- sh`; pinned pyinfra appends `-c` and the generated command. This authenticates every operation as the original bootstrap administrator without relying on a cached sudo timestamp or the `homelab`-only `NOPASSWD` policy. The reusable input resets for each generated command while keeping the secret absent from command arguments, environment variables, files, logs, and representations.
...
The deploy uses native `apt.packages` on Debian and Ubuntu and native `pacman.packages` on Arch Linux to install OpenSSH and sudo while preserving repository refresh behavior. Native `server.user` creates or reconciles `homelab` with `/bin/sh`, `/home/homelab`, and normal home skeleton creation. Pinned pyinfra selects an existing same-name group when creating a new user. Home reconciliation is disabled inside `server.user`: the deploy reads the pre-operation `Users` fact, preserves an existing user's primary group, predicts `homelab` for a new user, and applies owner, group, and mode `0755` through native `files.directory`. Operations run in package, user, home, validated sudoers, then SSH trust order. The sudoers operation validates the fixed policy with `visudo`, then atomically reconciles content, root ownership, mode `0440`, and non-directory file-type drift. A real directory at the sudoers target fails closed for operator remediation; a symlink, including one resolving to a directory, remains a replaceable directory entry.

The deploy retains one narrow privileged shell transaction for coordinated SSH trust changes because native file operations cannot validate the prospective multi-file sshd configuration before installation. It stages and validates the supplied user CA key, validates a candidate sshd configuration, and atomically reconciles `/etc/ssh/trusted-user-ca-keys.pem` and `/etc/ssh/sshd_config.d/90-homelab-user-ca.conf` as root-owned regular files with mode `0644`. Content, ownership, mode, and non-directory file-type drift are repaired. Before changing either SSH target, the transaction fails closed if either target is a real directory; symlink directory entries remain replaceable. After installation it validates the final sshd configuration and captures at most 131073 bytes from `sshd -T`, using the final byte as an overflow sentinel. It requires successful `sshd -T`, no overflow, and the intended effective `trustedusercakeys` path, failing when the preserved base configuration does not load the drop-in. It reloads only an already-running `ssh` service on Debian or Ubuntu or `sshd` service on Arch Linux, and only when trust content changed; metadata-only or file-type repair does not reload the service.
...
Stage values are `run_dir_create`, `keygen_spawn`, `keygen_exit`, `keygen_timeout`, `key_chmod`, `key_file_read`, `certificate_write`, `public_key_remove`, `known_hosts_write`, `inventory_serialize`, and `uv_spawn`. Classification values are `not_found`, `permission_denied`, `already_exists`, `timeout`, `nonzero_exit`, `wait_failed`, `invalid_data`, and `other`.

Pinned pyinfra retains successful operation output in per-host metadata instead of writing it to a process stream. The `system-info` entrypoint registers a callback for its fixed operation before declaring it. After that operation succeeds, the callback reads metadata for the one validated inventory host and writes only captured operation stdout to process stdout, adding a terminating newline when needed. It does not write operation stderr. The Rust runner keeps this intended result in its existing bounded stdout capture and parses the complete fixed protocol.

After uv starts, a normal nonzero child exit emits the same safe event with stage `uv_exit`. Pinned pyinfra writes failed remote output to process stderr before its callback can run. When bounded stderr contains the exact fixed `HOMELAB_SYSTEM_INFO_V1_BEGIN` marker and lacks the exact fixed `HOMELAB_SYSTEM_INFO_V1_END` marker, classification is `remote_command`. This direct protocol evidence proves only that the fixed remote command started but did not complete, and takes precedence over generic stderr patterns. For every other stderr state, classification uses only fixed patterns from pinned pyinfra 3.5 and Paramiko 3.5.1 sources. Values are `ssh_host_key_verification`, `ssh_authentication`, `connection_unavailable`, `connection_timeout`, `name_resolution`, `runtime_setup`, and `other`. Paramiko combines refused and unreachable endpoints under one connection message, so diagnostics do not infer a narrower cause. Pinned uv 0.11.15 contributes no category because its packaged executable provides no reviewed static signature for these failures. Unmatched uv errors use `other`. A child wait failure uses `uv_exit` and `wait_failed` without an exit code.

The runtime retains stderr only in a fixed-capacity buffer preallocated to the existing 32 KiB limit. The buffer never reallocates within that bound. It zeroizes the allocation and read buffer after use. Marker classification inspects bounded stderr only for the two exact fixed byte strings and retains no additional diagnostic data. Diagnostics never log, return, serialize, hash, or include raw child output or a substring from it.
