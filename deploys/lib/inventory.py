...
def bootstrap_inventory() -> list[tuple[str, dict[str, Any]]]:
    value = _load_object()
    expected = {"version", "host", "ca_public_key_file"}
    if set(value) != expected or value["version"] != 1:
        raise ValueError("bootstrap inventory requires version 1, host, and ca_public_key_file")
    address, data = _host(value["host"])
    ca_path = _validate_path(value["ca_public_key_file"], "ca_public_key_file")
    ca_text = ca_path.read_text(encoding="ascii")
    if len(ca_text.encode()) > 16_384:
        raise ValueError("CA public key exceeds 16384 bytes")
    fields = ca_text.strip().split()
    if len(fields) < 2 or fields[0] not in _PUBLIC_KEY_TYPES:
        raise ValueError("ca_public_key_file must contain one OpenSSH public key")
    if "PRIVATE KEY" in ca_text or len(ca_text.splitlines()) != 1:
        raise ValueError("ca_public_key_file must not contain private or multiple keys")
    data["homelab_ca_public_key"] = ca_text.strip() + "\n"
    return [(address, data)]
